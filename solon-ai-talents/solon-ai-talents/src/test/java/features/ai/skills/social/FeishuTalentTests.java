...
import org.noear.solon.ai.talents.social.FeishuTalent;
import org.noear.solon.ai.talents.social.WeComTalent;

/**
 * 社交/通知类工具包单元测试：验证逻辑、签名与报文结构
 */
public class FeishuTalentTests {
    private final String apiUrl = "https://open.feishu.cn/open-apis/bot/v2/hook/5b9a04a5-e051-420a-94b2-9b1473c4ec16";

    public FeishuTalent getSocialTalent() {
        return new FeishuTalent(apiUrl);
    }

    @Test
    public void testMarkdownLogic() {
        FeishuTalent skill = getSocialTalent();
        String result = skill.send("测试标题", "测试内容");
    ...
    public void testAgentDrivenNotification() throws Throwable {
        ...
        SimpleAgent agent = SimpleAgent.of(LlmUtil.getChatModel())
                .role("运维告警助手")
                .defaultTalentAdd(getSocialTalent())
                .build();
        ...
        String query = "这是刚刚捕获的日志：\n" + errorLogs +
                "\n请分析原因并总结成简短的告警消息，发送到飞书。标题固定为 '数据库异常告警'。";
