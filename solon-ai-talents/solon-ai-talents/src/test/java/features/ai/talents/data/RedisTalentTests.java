package features.ai.talents.data;
...
import org.noear.solon.ai.talents.data.RedisTalent;
 ...
 */
@SolonTest
public class RedisTalentTests {

    private RedisTalent redisTalent;

    @BeforeEach
    public void setup() {
        // 从容器获取或根据配置实例化（假设测试环境已配置 redis）
        Properties properties = Solon.cfg().getProp("solon.redis");
        redisTalent = new RedisTalent(new RedisClient(properties));
    }
    ...
    public void testToolMethodDirectly() {
        ...
        // 测试存储
        String setRet = redisTalent.set(key, value);
        Assertions.assertTrue(setRet.contains("记在我的笔记里了"));

        // 测试读取
        String getRet = redisTalent.get(key);
        Assertions.assertTrue(getRet.contains(value));

        // 测试读取不存在的键
        String getNone = redisTalent.get("none_key");
