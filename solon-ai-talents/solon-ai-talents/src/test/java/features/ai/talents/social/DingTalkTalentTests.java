...
import org.noear.solon.ai.talents.social.DingTalkTalent;

/**
 * 社交/通知类工具包单元测试：验证逻辑、签名与报文结构
 */
public class DingTalkTalentTests {
    ...
    public DingTalkTalent getSocialTalent(){
        return new DingTalkTalent(apiUrl);
    }

    @Test
    public void testMarkdownLogic() {
        DingTalkTalent skill = getSocialTalent();
    ...
    public void testAgentDrivenNotification() throws Throwable {
        ...
        String query = "这是刚刚捕获的日志：\n" + errorLogs +
                "\n请分析原因并总结成简短的告警消息，发送到钉钉。标题固定为 '数据库异常告警'。";
