package features.ai.talents.memory;
 ...
 */
@SolonTest
public class MemTalentTests {

//    private MemoryTalent memTalent;
...
//
//        // 3. 实例化 MemTalent
//        memTalent = new MemoryTalent(new MemoryStoreProviderReadisImpl(redisClient), mockSearchProvider);
//
//        // 清理历史数据防止干扰
//        memTalent.prune("test_hobby", testUserId);
//        memTalent.prune("user_profile", testUserId);
//    }
//
...
//
//        // 第一次存储：存入初始信息
//        String ret1 = memTalent.extract(key, "喜欢游泳", 5, testUserId);
//        Assertions.assertTrue(ret1.contains("操作成功"));
//
//        // 第二次存储：触发“认知对比”
//        String ret2 = memTalent.extract(key, "现在更喜欢滑雪了", 8, testUserId);
//
...
//
//        // 调用搜索工具
//        String searchRet = memTalent.search("职业", testUserId);
//
...
//        // 预存两个碎片
//        memTalent.extract("frag_1", "喜欢红色", 3, testUserId);
//        memTalent.extract("frag_2", "喜欢跑车", 3, testUserId);
//
//        // 执行整合
//        String consRet = memTalent.consolidate(
...
//
//        // 验证旧碎片已被修剪 (Prune)
//        String recallNone = memTalent.recall("frag_1", testUserId);
//        Assertions.assertTrue(recallNone.contains("未找到"));
//
//        // 验证新认知已存入
//        String recallNew = memTalent.recall("car_pref", testUserId);
...
//        SimpleResponse resp = agent.prompt("别研究 Solon AI 了，帮我看看 MemTalent 怎么实现。")
//                .session(session)
//                .call();
//
//        System.out.println("AI 进化回复: " + resp.getContent());
//        // 期望 AI 回复中包含类似：“好的，已更新认知。我记得你刚才还在看框架，现在重点转向 MemTalent 了。”
//    }
//
//    @Test
...
//                .session(session)
//                .options(o -> o.talentAdd(memTalent))
//                .call();
//
//        // 步骤 2：变更信息（Agent 调用 mem_extract，触发反馈后应在回复中体现反思）
//        ChatResponse resp = chatModel.prompt("别研究 Solon AI 了，帮我看看 MemTalent 怎么实现。")
//                .session(session)
//                .options(o -> o.talentAdd(memTalent))
//                .call();
//
