...
public class ToolGatewayTalentTest {
    private ToolGatewayTalent gatewayTalent;
    ...
    void setUp() {
        ...
        gatewayTalent = new ToolGatewayTalent();
        for (FunctionTool tool : mockTools) {
            gatewayTalent.addTool(tool);
        }
    }
    ...
    void testFullMode() {
        gatewayTalent.dynamicThreshold(5); // 3 < 5

        Collection<FunctionTool> tools = gatewayTalent.getTools(Prompt.of(""));

        // 断言：FULL模式下直接暴露业务工具
        assertEquals(3, tools.size());
        assertTrue(tools.stream().anyMatch(t -> t.name().equals("adder")));

        String instruction = gatewayTalent.getInstruction(Prompt.of(""));
    ...
    void testSummaryMode() {
        gatewayTalent.dynamicThreshold(1);  // 3 > 1，超过 dynamicThreshold
        gatewayTalent.listThreshold(5);     // 3 < 5，未超过 listThreshold → SUMMARY 模式
    ...
    void testListMode() {
        gatewayTalent.dynamicThreshold(1);  // 3 > 1
        gatewayTalent.listThreshold(2);     // 3 > 2，超过 listThreshold
        gatewayTalent.searchThreshold(5);   // 3 < 5，未超过 searchThreshold → LIST 模式
    ...
    void testSearchMode() {
        gatewayTalent.dynamicThreshold(1);
        gatewayTalent.listThreshold(2);
        gatewayTalent.searchThreshold(2); // 3 > 2，触发 SEARCH 模式
    ...
    void testSearchTools() {
        ...
        Object result = gatewayTalent.searchTools("weather");
        ...
        // 搜索不存在的关键词
        Object emptyResult = gatewayTalent.searchTools("unknown_keyword");
    ...
    void testGetToolDetail() {
        String detail = gatewayTalent.getToolDetail("adder");
    ...
    void testCallTool() {
        ...
        ToolResult result = gatewayTalent.callTool("adder", args);
    ...
    void testCaseInsensitivity() {
        // 注册时是小写，调用时用大写
        ToolResult result = gatewayTalent.callTool("ADDER", new HashMap<>());
