package org.noear.solon.ai.talents.generation;
...
/**
 * 视频生成工具包：为 AI 提供多模态视频创作能力。
 *
 * <p>该工具包集成了先进的视频生成驱动（如 Sora, Kling, Luma 等），支持根据文本描述合成短视频。
 * 核心设计要点：
 * <ul>
 * <li><b>成本感知</b>：通过意图识别，仅在明确需要视频创作时触发，避免昂贵的 Token/API 浪费。</li>
 * <li><b>规格约束</b>：支持动态指定时长与宽高比，适配社交媒体（9:16）或传统荧幕（16:9）。</li>
 * <li><b>异步兼容性</b>：考虑到视频生成的长耗时特征，驱动接口建议实现为内部轮询至下载完成，确保 Agent 拿到的是最终可用的本地物理文件。</li>
 * </ul>
 * </p>
 *
 * @author noear
 ...
 */
@Preview("3.9.1")
public class VideoGenerationTalent extends AbsTalent {
    private static final Logger LOG = LoggerFactory.getLogger(VideoGenerationTalent.class);

    private final VideoDriver driver;
    private final String apiKey;
    private final Path rootPath;

    public VideoGenerationTalent(VideoDriver driver, String apiKey, String workDir) {
        this.driver = driver;
        this.apiKey = apiKey;
