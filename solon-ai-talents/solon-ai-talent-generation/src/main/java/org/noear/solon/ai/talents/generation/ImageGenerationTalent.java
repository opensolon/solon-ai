...
/**
 * 绘图生成工具包：为 AI 提供多模态视觉创作能力。
 *
 * <p>该工具包集成了多种绘图驱动（如 DALL-E, Midjourney 等），允许 Agent 根据自然语言描述生成图像并持久化到本地。
 * 核心特性：
 * <ul>
 * <li><b>智能命名</b>：支持 AI 建议文件名，并自动处理物理文件名冲突。</li>
 * <li><b>安全沙箱</b>：强制生成物存储在指定的 WorkDir，防止路径逃逸。</li>
 * <li><b>链式友好</b>：显式返回保存后的物理路径，方便后续工具包（如文件打包、邮件发送）直接引用。</li>
 * </ul>
 * </p>
 *
 * @author noear
 ...
 */
@Preview("3.9.1")
public class ImageGenerationTalent extends AbsTalent {
    private static final Logger LOG = LoggerFactory.getLogger(ImageGenerationTalent.class);

    private final ImageDriver driver;
    private final String apiKey;
    private final Path rootPath;

    public ImageGenerationTalent(ImageDriver driver, String apiKey, String workDir) {
        this.driver = driver;
        this.apiKey = apiKey;
