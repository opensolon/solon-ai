...
* 自动对接：只需提供 OpenAPI JSON 地址，自动生成 AI 工具包。
* 自适应模式：接口少时全量注入 (FULL)，接口多时自动切换为分步导航模式（SUMMARY/LIST/SEARCH），节省 Token。
...
public class Demo {
    public void test(ChatModel chatModel) throws Throwable {
        ...
        // 2. 创建工具包
        OpenApiTalent apiSkill = new OpenApiTalent()
                .addApi(docUrl, baseUrl);
