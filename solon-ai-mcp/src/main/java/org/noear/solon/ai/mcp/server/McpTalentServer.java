 ...
 * 机制：利用注解将才能的生命周期方法（isSupported, getInstruction）映射为 MCP 的 Tool 或 Resource，
 * 供远程 {@link McpTalentClient} 发现并调用。
 *
 * @author noear
 * @since 3.9.0
 */
...
public abstract class McpTalentServer implements Talent {
     ...
     * 导出才能元数据作为 MCP 资源
     */
    @ResourceMapping(uri = "talent://metadataMcp", meta = "{hide:1}")
