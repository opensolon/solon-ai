...
public class McpStreamableServerSession implements McpLoggableSession {
    ...
    public Mono<Void> responseStream(McpSchema.JSONRPCRequest jsonrpcRequest, McpStreamableServerTransport transport) {
        return Mono.deferContextual(ctx -> {
            ...
            if (requestHandler == null) {
                ...
                return transport
                        .sendMessage(new McpSchema.JSONRPCResponse(McpSchema.JSONRPC_VERSION, jsonrpcRequest.getId(), null,
            ...
            return requestHandler
                    .handle(new McpAsyncServerExchange(this.id, stream, clientCapabilities.get(), clientInfo.get(),
                            transportContext), jsonrpcRequest.getParams())
                    .map(result -> new McpSchema.JSONRPCResponse(McpSchema.JSONRPC_VERSION, jsonrpcRequest.getId(), result,
                            null))
                    .onErrorResume(e -> {
                        var errorResponse = new McpSchema.JSONRPCResponse(McpSchema.JSONRPC_VERSION, jsonrpcRequest.getId(),
    ...
    public Mono<Void> accept(McpSchema.JSONRPCNotification notification) {
        return Mono.deferContextual(ctx -> {
            ...
            return notificationHandler.handle(new McpAsyncServerExchange(this.id, listeningStream,
                    this.clientCapabilities.get(), this.clientInfo.get(), transportContext), notification.getParams());
        });
    ...
    public Mono<Void> accept(McpSchema.JSONRPCResponse response) {
        return Mono.defer(() -> {
            var stream = this.requestIdToStream.get(response.getId());
            if (stream == null) {
