...
public class McpAsyncServer {
	 ...
	 */
	public McpAsyncServer(McpServerTransportProvider mcpTransportProvider, ObjectMapper objectMapper,
	...
	public McpAsyncServer(McpStreamableServerTransportProvider mcpTransportProvider, ObjectMapper objectMapper,
				   McpServerFeatures.Async features, Duration requestTimeout,
				   McpUriTemplateManagerFactory uriTemplateManagerFactory, JsonSchemaValidator jsonSchemaValidator) {
		this.mcpTransportProvider = mcpTransportProvider;
		this.objectMapper = objectMapper;
	...
	private static McpServerFeatures.AsyncToolSpecification withStructuredOutputHandling(
		...
		return McpServerFeatures.AsyncToolSpecification.builder()
				.tool(toolSpecification.getTool())
				.callHandler(new StructuredOutputCallToolHandler(jsonSchemaValidator,
						toolSpecification.getTool().getOutputSchema(), toolSpecification.getCallHandler()))
				.build();
	}
	...
	private McpRequestHandler<CallToolResult> toolsCallRequestHandler() {
		return (exchange, params) -> {
			...
			return toolSpecification.map(tool -> Mono.defer(() -> tool.getCallHandler().apply(exchange, callToolRequest)))
	...
	private List<McpSchema.ResourceTemplate> getResourceTemplates() {
		...
		List<ResourceTemplate> resourceTemplates = this.resources.keySet()
				...
				.map(uri -> {
					...
					var template = new McpSchema.ResourceTemplate(resource.getUri(), resource.getName(), resource.getTitle(),
