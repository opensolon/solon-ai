...
- [1. 架构总览](#1-架构总览)
- [2. 快速开始](#2-快速开始)
- [3. 权限规则 (PermissionRule)](#3-权限规则-permissionrule)
- [4. 权限上下文 (PermissionContext)](#4-权限上下文-permissioncontext)
- [5. 权限评估引擎 (PermissionEngine)](#5-权限评估引擎-permissionengine)
- [7. 内置干预策略](#7-内置干预策略)
...
```
                     ...
                     │                   HarnessEngine                      │
                     │                                                      │
                     │  全局 PermissionContext (rules)                       │
                     │        │                                              │
                     │        ├── AgentFactory.create()                     │
                     │        │     读取 Agent 定义的 permissionRules        │
                     │        │     → 构建 delta                             │
...
| `PermissionEngine` | solon-ai-harness | 权限评估引擎（priority 越大越优先，同级 DENY > ALLOW > ASK） |
| `PermissionContext` | solon-ai-harness | 权限上下文（规则 + 工作目录，不可变） |
| `PermissionRule` | solon-ai-harness | 权限规则（工具名 + glob 模式 + 行为 + 优先级） |
...
| `AgentDefinition.Metadata` | solon-ai-harness | Agent 定义元数据（可配置 permissionRules） |
...
策略评估时 resolveContext(trace):
  ...
  3. 若 agentCtx.rules() 非空 → 追加到全局 rules
  4. 返回合并后的上下文给 PermissionEngine
```
...
// 2. 人工决策 — 批准并修正金额（主路径：按 HITLTask 提交）
...
`PermissionRule` 将工具名（含可选 glob 模式）映射到一种权限行为，并通过 `priority` 支持默认规则与例外规则。

| 默认效果 | 推荐规则 |
|----------|----------|
| 默认询问 | 不配置规则，或配置 `PermissionRule.ask("*").priority(-100)` |
| 默认放行 | `PermissionRule.allow("*").priority(-100)` |
| 默认拒绝 | `PermissionRule.deny("*").priority(-100)` |
| 高优先级例外 | 额外添加更高 `priority` 的细粒度规则 |

示例：默认询问，但允许 `git status`，拒绝 `rm -rf *`：

```java
HarnessEngine.of(workspace, home)
    .permissionRuleAdd(PermissionRule.ask("*").priority(-100))
    .permissionRuleAdd(PermissionRule.allow("bash", "git status").priority(10))
    .permissionRuleAdd(PermissionRule.deny("bash", "rm -rf *").priority(100))
    .build();
```
...
// 优先级规则 — 数值越大越优先
PermissionRule defaultAsk = PermissionRule.ask("*").priority(-100);
PermissionRule highPriorityDeny = PermissionRule.deny("bash", "rm -rf *").priority(100);
...
`PermissionContext` 是不可变对象，包含规则列表、工作目录和附加目录。所有变更方法返回新实例。
...
```
1. priority 越大越优先
2. 同 priority 下 DENY > ALLOW > ASK
3. 无规则匹配时返回 ASK
```

### 6.2 默认规则

默认行为通过低优先级通配规则表达，例如 `PermissionRule.ask("*").priority(-100)`。
...
PermissionContext ctx = PermissionContext.builder()
    .addRule(PermissionRule.ask("*").priority(-100))
    .addRule(PermissionRule.deny("bash", "rm -rf *").priority(100))
    .addRule(PermissionRule.allow("bash", "git status").priority(10))
    .build();
...
Agent 定义（Markdown 文件，含 YAML Front Matter）支持声明自己的权限规则。这些规则会作为 **delta** 通过 `AgentFactory` 注入到子 Agent 的 `Attrs` 中，在 HITL 策略评估时与全局规则合并。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `permissionRules` | List | Agent 级细粒度权限规则，支持 `priority` |
...
AgentFactory.create()
  │
  ├── 检查 metadata.hasPermissionRules()
  │     └── 有 → 构建 PermissionContext delta → addRules(rules)
  │
...
HITL 策略 resolveContext(trace):
  ...
  │     └── delta.rules() 非空 → 追加到全局 rules
  └── 返回合并后的 PermissionContext
```

> **关键语义**：
> - Agent 级 `permissionRules` 与全局规则合并后统一按 `priority` 评估
> - `priority` 越大越优先，同优先级下 `DENY > ALLOW > ASK`
> - 默认行为建议使用低优先级通配规则，例如 `priority: -100`
...
// 或直接通过 Metadata 设置
definition.getMetadata().addPermissionRule(PermissionRule.deny("*").priority(-100));
definition.getMetadata().addPermissionRule(PermissionRule.allow("read").priority(10));
...
// 获取当前挂起的任务（批场景返回第一个；完整列表用 getPendingTasks）
HITLTask task = HITL.getPendingTask(session);
if (task != null) {
    String callUuid = task.getCallUuid();    // 调用实例主键
...
// 按主键 / 工具名定位（toolName 在批内须唯一；多实例用 callUuid 或 getPendingTasksByToolName）
HITLTask t1 = HITL.getPendingTaskByCallUuid(session, callUuid);
HITLTask t2 = HITL.getPendingTaskByToolName(session, "bash");
List<HITLTask> all = HITL.getPendingTasks(session);
```

### 10.2 决策回填（主路径：HITLTask）
...
HITL.approve(session, task, true);

// 批准 + 备注
HITL.approve(session, task, "已确认安全");

// 拒绝（终止流程，Agent 输出拒绝理由）
HITL.reject(session, task);

// 拒绝 + 理由
HITL.reject(session, task, "禁止在生产环境执行此操作");

// 跳过（不执行工具，但向 Agent 返回跳过原因，流程继续）
HITL.skip(session, task);

// 跳过 + 原因
HITL.skip(session, task, "该步骤暂不执行，请继续下一步");

// 批：全部批准 / 全部拒绝
HITL.approveAll(session);
HITL.rejectAll(session, "批量驳回");
```
...
当用户通过 `HITL.approve(session, task, true)` 批准并标记 `alwaysAllow = true` 时，系统会自动向 `PermissionContext` 注入一条会话级规则，使后续同类操作自动放行。
...
public class AgentController {
    ...
    public String reject(@Body RejectRequest req) {
        ...
        HITL.reject(session, task, req.getReason());
    ...
    public String skip(@Body SkipRequest req) {
        ...
        HITL.skip(session, task, req.getReason());
...
| `HITL.getPendingTasks(session)` | 获取本轮全部挂起任务 |
| `HITL.getPendingTask(session)` | 获取挂起任务（批中第一个） |
| `HITL.getPendingTaskByCallUuid(session, callUuid)` | 按调用实例主键获取挂起任务 |
| `HITL.getPendingTaskByToolName(session, toolName)` | 按工具名获取挂起任务（批内须唯一） |
| `HITL.getPendingTasksByToolName(session, toolName)` | 按工具名获取全部挂起任务 |
| `HITL.getDecision(session, task)` | 获取指定任务的决策 |
| `HITL.approve(session, task)` | 批准 |
| `HITL.approve(session, task, alwaysAllow)` | 批准 + 始终允许 |
| `HITL.approve(session, task, comment)` | 批准 + 备注 |
| `HITL.reject(session, task)` | 拒绝 |
| `HITL.reject(session, task, comment)` | 拒绝 + 理由 |
| `HITL.skip(session, task)` | 跳过 |
| `HITL.skip(session, task, comment)` | 跳过 + 原因 |
| `HITL.submit(session, task, decision)` | 提交完整决策实体 |
| `HITL.approveAll(session)` | 批准全部挂起任务 |
| `HITL.rejectAll(session, comment)` | 拒绝全部挂起任务 |
...
| `PermissionRule.of(toolName, behavior, priority)` | 通用工厂（无模式 + 优先级） |
| `PermissionRule.withPattern(toolName, behavior, pattern)` | 通用工厂（带 glob 模式） |
| `PermissionRule.withPattern(toolName, behavior, pattern, priority)` | 通用工厂（带 glob 模式 + 优先级） |
| `.priority(priority)` | 返回指定优先级的新规则实例 |
