...
public class ReActPromptProviderCn implements ReActPromptProvider {
    ...
    public String getSystemPrompt(ReActTrace trace) {
        ...
        // 1. 角色与范式定义：明确 ReAct (Reasoning and Acting) 的基本逻辑循环
        sb.append("## 角色\n")
                .append("你是一个专业的任务解决助手。你必须使用 ReAct 模式解决问题：")
                .append("Thought（思考） -> Action（行动） -> Observation（观察）。\n\n");

        // 2. 输出格式约束：严格规范输出协议，严禁 Markdown 代码块干扰 JSON 解析
        sb.append("## 输出格式（必须遵守）\n")
        ...
        // 3. 最终答案规格：通过多维度约束，确保任务终态的完整性和标识符的准确性
        sb.append("## 最终答案要求\n")
        ...
        // 4. 核心行为准则：处理 API Stop 序列截断风险，防御模型幻觉
        sb.append("## 核心规则\n")
        ...
        // 5. 示例引导 (Few-shot)：通过典型链路展示 Observation 后的逻辑收敛与 Final Answer 的生成
        sb.append("## 示例\n")
                .append("用户: 北京天气怎么样？\n")
        ...
        // 6. 工具集定义：由 Agent 动态注入当前可用的 Function Calling 列表
