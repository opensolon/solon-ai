...
public class HITLIndustrialTest {
    ...
    public void testRejectFlow() throws Throwable {
        ...
        // 验证被拦截
        Assertions.assertTrue(resp1.getSession().isPending(), "应该是被中断状态");
        ...
        Assertions.assertFalse(resp2.getSession().isPending(), "不应该被中断状态（而是结束）");
    ...
    public void testSkipFlow() throws Throwable {
        ...
        ReActResponse resp1 = agent.prompt(prompt).session(session).call();
        Assertions.assertTrue(resp1.getSession().isPending());
        ...
        Assertions.assertFalse(resp2.getSession().isPending(), "流程应正常结束");
