...
public class MemSkillTests {
    ...
    public void setup() {
        ...
        // 3. 实例化 MemSkill
        memSkill = new MemSkill(redisClient, testUserId, mockSearchProvider);

        // 清理历史数据防止干扰
        memSkill.prune("test_hobby");
        memSkill.prune("user_profile");
    }
    ...
    public void testExtractWithReflection() {
        ...
        // 第一次存储：存入初始信息
        String ret1 = memSkill.extract(key, "喜欢游泳", 5);
        Assertions.assertTrue(ret1.contains("操作成功"));

        // 第二次存储：触发“认知对比”
        String ret2 = memSkill.extract(key, "现在更喜欢滑雪了", 8);
    ...
    public void testSearchIntegration() {
        ...
        // 调用搜索工具
        String searchRet = memSkill.search("职业");
    ...
    public void testConsolidate() {
        // 预存两个碎片
        memSkill.extract("frag_1", "喜欢红色", 3);
        memSkill.extract("frag_2", "喜欢跑车", 3);
        ...
        // 验证旧碎片已被修剪 (Prune)
        String recallNone = memSkill.recall("frag_1");
        Assertions.assertTrue(recallNone.contains("未找到"));

        // 验证新认知已存入
        String recallNew = memSkill.recall("car_pref");
    ...
    public void testAgentMindEvolution() throws Throwable {
        ...
        agent.prompt("记一下，我最近在研究 Solon AI 框架。").call();

        // 步骤 2：变更信息（Agent 调用 mem_extract，触发反馈后应在回复中体现反思）
        SimpleResponse resp = agent.prompt("别研究 Solon AI 了，帮我看看 MemSkill 怎么实现。").call();
